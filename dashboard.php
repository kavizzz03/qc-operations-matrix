...
// Load user's tasks into session if not already loaded
...
function canAccess($task_code) {
    ...
    if (function_exists('userHasTask')) {
        return userHasTask($pdo, $_SESSION['user_id'], $task_code);
    }
    return true; // Default to true if function doesn't exist
}
...
$queryStr = "SELECT m.*, s.supplier_name, s.email, s.contact_number, mo.mode_name, re.reason_name 
             FROM qc_damage_main m 
             JOIN suppliers s ON m.supplier_id = s.supplier_id 
             LEFT JOIN qc_modes mo ON m.mode_id = mo.mode_id
             LEFT JOIN qc_reasons re ON m.reason_id = re.reason_id
...
<head>
    ...
    <style>
        ...
        @media print { .no-print { display: none !important; } #detailModal { position: relative !important; display: block !important; background: white !important; color: black !important; } }
    </style>
</head>
        ...
        <div class="flex flex-1">
            ...
            <aside class="w-72 bg-[#0f172a] border-r border-slate-800 flex flex-col fixed h-full z-40 no-print overflow-y-auto custom-scroll">
                ...
                <nav class="flex-1 px-6 space-y-1.5 pb-6">
                    ...
                    <!-- Internal Resource Management Framework -->
                    ...
                    <!-- SYSTEM ADMINISTRATION CONTEXT ENGINE -->
                    ...
                    <div class="pt-4 border-t border-slate-800/50 mt-2">
                        ...
                        <a href="users.php" class="flex items-center p-3 hover:bg-slate-800/50 rounded-xl transition group text-slate-400 hover:text-white">
                            <span class="text-[10px] font-bold uppercase tracking-wider">+ Deploy New User</span>
                        </a>
                        <?php endif; ?>
                        ...
                        <a href="qc_change_password.php" class="flex items-center p-3 hover:bg-slate-800/50 rounded-xl transition group text-slate-400 hover:text-white">
                            <span class="text-[10px] font-bold uppercase tracking-wider">⚙ Change Credentials</span>
                        </a>
                        <?php endif; ?>
                    </div>
                    <?php endif; ?>
            ...
            <main class="flex-1 ml-72 p-10">
                <header class="flex justify-between items-start mb-12 no-print">
                    <div>
                        ...
                        <p class="text-slate-500 text-[10px] font-black uppercase tracking-[0.4em] mt-3 flex items-center">
                            ...
                            Active Records: <?= number_format($totalRecords) ?>
                        </p>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10 no-print">
                    <div class="stat-card bg-gradient-to-br from-red-600/20 to-transparent p-8 rounded-[2.5rem] border border-red-900/30 shadow-xl relative overflow-hidden group cursor-pointer">
                    ...
                    <div class="stat-card bg-gradient-to-br from-blue-600/20 to-transparent p-8 rounded-[2.5rem] border border-blue-900/30 shadow-xl relative overflow-hidden group cursor-pointer">
                        <div class="relative z-10">
                            <p class="text-[10px] font-black text-blue-500 uppercase tracking-widest">Pending Tasks</p>
                    ...
                    <div class="stat-card bg-gradient-to-br from-green-600/20 to-transparent p-8 rounded-[2.5rem] border border-green-900/30 shadow-xl relative overflow-hidden group cursor-pointer">
                        <div class="relative z-10">
                ...
                <div class="mb-10 no-print">
                    <form class="flex gap-4 bg-slate-900/50 p-4 rounded-[2.5rem] border border-slate-800 backdrop-blur-xl shadow-2xl">
                                    ...
                                    <tr class="hover:bg-red-600/[0.03] transition-all group cursor-pointer" onclick="viewAudit(<?= $row['record_id'] ?>)">
                                        ...
                                        <td class="px-10 py-8">
                                            <div class="flex items-center">
                                                <?php 
                                                $steps = [
                                                    ['col' => 'is_informed', 'color' => 'bg-green-500', 'title' => 'Supplier Informed'],
                                                    ['col' => 'is_store_received', 'color' => 'bg-blue-500', 'title' => 'Store Received'],
                                                    ['col' => 'is_gate_cleared', 'color' => 'bg-yellow-500', 'title' => 'Gate Passed'],
                                                    ['col' => 'is_handover_complete', 'color' => 'bg-red-500', 'title' => 'Return Complete']
                                                ];
                                                foreach($steps as $index => $step): 
                                                    $active = $row[$step['col']];
                                                ?>
                                                    <div onclick="updateFlag(event, <?= $row['record_id'] ?>, '<?= $step['col'] ?>')" 
                                                         title="<?= $step['title'] ?>"
                                                         class="w-6 h-6 rounded-lg border-2 transition-all cursor-pointer flex items-center justify-center
                                                         <?= $active ? "$step[color] border-transparent text-white dot-active" : "border-slate-800 bg-slate-900 text-transparent" ?>">
                                        ...
                                        <td class="px-10 py-8 text-right">
                                            <button class="text-[10px] font-black uppercase text-slate-600 group-hover:text-red-500 tracking-widest border border-slate-800 group-hover:border-red-600/30 px-6 py-3 rounded-xl transition-all">Audit View</button>
                                        </td>
                                    </tr>
                                    <?php endforeach; ?>
                                <?php else: ?>
                ...
                <div class="mt-10 flex justify-between items-center px-6 no-print">
                    <p class="text-[10px] font-black text-slate-600 uppercase tracking-widest italic">Segment <?= $page ?> of <?= $totalPages ?></p>
        ...
        <div class="bg-[#0f172a] border border-slate-800 w-full max-w-6xl max-h-[90vh] overflow-hidden rounded-[4rem] shadow-2xl flex flex-col relative">
            ...
            <div class="p-12 overflow-y-auto custom-scroll">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-12" id="printableArea">
                    <!-- Column 1: Supplier & Structural Context -->
                    <div class="space-y-6">
            ...
            <div class="p-12 pt-0 no-print flex justify-end gap-4 mt-auto">
                <button onclick="window.print()" class="bg-white text-black px-12 py-5 rounded-[2rem] text-[10px] font-black uppercase tracking-[0.2em] hover:bg-red-600 hover:text-white transition-all shadow-2xl">Print Logistics Note</button>
            </div>
        </div>
    </div>

    <!-- CORE SYSTEM RUNTIME INTERFACES -->
    <script>
        // Real-time Clock Pipeline
        function updateTime() {
        ...
        // Audit View Hydration Template Logic
        async function viewAudit(id) {
            try {
                ...
                document.getElementById('mEmail').innerText = "E: " + (data.main.email || "NOT SET");
                document.getElementById('mPhone').innerText = "P: " + (data.main.contact_number || "NOT SET");
                
                // Binding Extended Structural Elements
                ...
                document.getElementById('mReason').innerText = data.main.reason_name || "No explicit error statement flagged.";
                ...
                document.getElementById('mItems').innerHTML = data.items.map(i => `
                    <div class="flex justify-between items-center bg-slate-900 p-4 rounded-2xl border border-slate-800/50">
                        <div>
                            <p class="text-white font-black text-xs tracking-tighter uppercase">${i.item_code}</p>
                            <p class="text-[8px] text-slate-600 font-bold uppercase mt-0.5 italic">Product Unit</p>
                        </div>
                ...
                Swal.fire({ icon: 'error', title: 'Fetch Error', text: 'Could not load record metrics.', background: '#0f172a', color: '#fff' });
            }
        }
        ...
        async function updateFlag(event, id, column) {
            ...
            const { value: confirm } = await Swal.fire({
                ...
                text: "Advance this record across the operations tree status pipeline?",
                icon: 'question',
                showCancelButton: true,
                background: '#0f172a', color: '#fff', confirmButtonColor: '#dc2626', cancelButtonColor: '#1e293b',
                confirmButtonText: 'Yes, Advance Status'
            });
            ...
            try {
                const res = await fetch('update_flag.php', {
                    ...
                    body: `id=${id}&column=${column}`
                });
                const data = await res.json();
                if (data.success) {
                    await Swal.fire({ icon: 'success', title: 'Pipeline Advanced', toast: true, position: 'top-end', showConfirmButton: false, timer: 1000, background: '#0f172a', color: '#fff' });
                    window.location.reload(); 
                } else {
                    Swal.fire({ icon: 'error', title: 'Validation Lockout', text: data.message, background: '#0f172a', color: '#fff' });
                }
        ...
        function setupCommButton(btnId, hasValue, isSent) {
            ...
            if (!hasValue || hasValue === "NOT SET") {
                btn.disabled = true; btn.innerText = "NO DATA";
                ...
                btn.disabled = false;
                btn.innerText = isSent == 1 ? "Resend Update" : "Dispatch Note";
                btn.className = isSent == 1 
                    ? "w-full py-4 rounded-xl border border-green-500/30 text-green-500 hover:bg-green-500 hover:text-white text-[10px] font-black uppercase tracking-widest transition-all" 
                    : "w-full py-4 rounded-xl bg-red-600 text-white hover:bg-red-700 text-[10px] font-black uppercase tracking-widest shadow-xl transition-all";
            }
        }

        async function handleComms(type) {
            const { value: pass } = await Swal.fire({ 
                title: 'Authorize Dispatch', input: 'password', inputPlaceholder: 'System Key',
            ...
            .then(data => {
                if (data.success) {
                    Swal.fire({icon: 'success', title: 'Notification Dispatched', background: '#0f172a', color: '#fff'})
                    .then(() => viewAudit(window.currentRecord.record_id));
                } else {
                    Swal.fire({icon: 'error', title: 'Access Denied', text: data.message, background: '#0f172a', color: '#fff'});
                }
            });
        }
