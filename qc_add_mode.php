...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($mode_name)) {
        $message = 'Audit mode name is required.';
        $status = 'error';
    } else {
        try {
            // Check for duplication
            $check = $pdo->prepare("SELECT COUNT(*) FROM qc_modes WHERE LOWER(mode_name) = LOWER(?)");
            $check->execute([$mode_name]);
            
            if ($check->fetchColumn() > 0) {
                $message = 'This audit mode already exists in the registry.';
                $status = 'error';
            } else {
                // Insert into registry
                $stmt = $pdo->prepare("INSERT INTO qc_modes (mode_name) VALUES (?)");
                $stmt->execute([$mode_name]);
                $message = 'Audit mode registered successfully!';
                $status = 'success';
            }
        } catch (Exception $e) {
...
<head>
    <meta charset="UTF-8">
    <title>QC Engine | Add Audit Mode</title>
...
<body class="min-h-screen flex">
    ...
    <main class="ml-64 flex-1 p-10 flex flex-col justify-center items-center">
        <div class="w-full max-w-xl">
            <div class="mb-8">
                <h1 class="text-3xl font-black text-white tracking-tighter uppercase">Add Audit <span class="text-red-600">Mode</span></h1>
                <p class="text-slate-500 text-xs mt-1 uppercase tracking-widest font-bold">Register Workflow Channels</p>
            </div>

            <div class="glass p-8 rounded-[2.5rem] shadow-2xl border border-slate-800">
                <form method="POST" action="" class="space-y-6">
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Audit Mode Name</label>
                        <input type="text" name="mode_name" placeholder="e.g., Supplier Return, Factory Re-Audit" 
                               class="w-full bg-slate-900 border border-slate-800 rounded-xl px-5 py-4 text-sm text-white font-bold placeholder:text-slate-700 focus:ring-1 focus:ring-red-600 outline-none uppercase transition">
                    </div>
                    ...
                    <div class="flex gap-4 pt-4">
                        ...
                        <button type="submit" class="w-2/3 bg-red-600 hover:bg-red-700 text-white text-[10px] font-black uppercase tracking-wider py-4 rounded-xl transition shadow-lg shadow-red-600/20">Save Workflow Mode</button>
                    </div>
                </form>
            </div>
        </div>
